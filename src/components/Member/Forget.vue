  ...
  <div class="member-login">
    <section class="member-main">
      <div class="main-wrapper">
        ...
        <p  class="text"><i class="iconfont icon-mima icon3"></i><input type="password"  id="password" placeholder="设置新密码" maxlength="20" v-model="password" @blur="passwordTest"/></p>
        <p  class="text">
          <a href="javascript:;" class="finish" @click="sendFinish">下一步</a>
        </p>
      </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    data(){
      return {
        sendAuthCode:false,  //验证码状态
        time: 0,
        phone: '',
        authCode: '',
        password: '',
        finishState: false  //弹窗状态
      }
    },
    methods:{
      //获取短信验证码
      ...
      //提交
      sendFinish(){
        this.$http.post('http://47.94.249.58/ddyk/index.php/Api/sign/password',qs.stringify({phone:this.phone,password:this.password,authCode:this.authCode})).then(response=>{
          let res = response.data;
         /// console.log(res)
          if(res.flag == 'success'){
